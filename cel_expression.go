...
func compileCELProgram(data map[string]any, template Template, nativeTypes *nativeTypeSnapshot, trackState bool) (cel.Program, *cel.Ast, error) {
	...
	var typeAdapter ref.TypeAdapter = types.DefaultTypeAdapter
	envOptions := celEnvOptions(data, template, nativeTypes, func(value any) ref.Val {
		return typeAdapter.NativeToValue(value)
	})
	env, err := base.Extend(envOptions...)
	if err != nil {
		return nil, nil, err
	}
	typeAdapter = env.TypeAdapter()
...
func celEnvOptions(data map[string]any, template Template, nativeTypes *nativeTypeSnapshot, adaptValue func(any) ref.Val) []cel.EnvOption {
