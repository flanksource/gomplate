	...
	for _, benchmark := range cases {
		for _, optimize := range []bool{false, true} {
			...
			b.Run(name, func(b *testing.B) {
				data, err := Serialize(benchmark.data)
				if err != nil {
					b.Fatal(err)
				}
...
func registerBenchmarkNativeType(b *testing.B) {
	b.Helper()
	// The benchmark workflow compiles this file against both the void base API
	// and the error-returning head API.
	switch register := any(RegisterType).(type) {
	case func(any):
		register(benchmarkNativeInput{})
	case func(any) error:
		if err := register(benchmarkNativeInput{}); err != nil {
			b.Fatal(err)
		}
	default:
		b.Fatalf("unexpected RegisterType signature %T", RegisterType)
	}
}

func compileBenchmarkCELProgram(data map[string]any, expression string, optimize bool) (cel.Program, error) {
	env, err := cel.NewEnv(GetCelEnv(data)...)
	if err != nil {
		return nil, err
	}
