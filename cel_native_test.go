...
var _ = Describe("CEL native types", Ordered, func() {
	...
	It("adapts native values returned by template functions", func() {
		person := functionCELPerson{DisplayName: "Margaret Hamilton"}
		Expect(RegisterType(person)).To(Succeed())

		result, err := RunExpression(nil, Template{
			Expression: "person().display_name",
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(result).To(Equal("Margaret Hamilton"))
	})
